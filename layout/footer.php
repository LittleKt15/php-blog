...
if (isset($_POST['userSignUpBtn'])) {
    ...
    if ($name != '' && $email != '' && $password != '') {
        ...
        $result = $stmt->execute();
        if ($result) {
            echo "<script>sweetAlert('Sign up successfully.', 'index.php')</script>";
        }
    }
}
...
if (isset($_POST['signInBtn'])) {
    ...
    if ($user) {
        $_SESSION['user'] = $user;
        if ($user->role === 'admin') {
            echo "<script>sweetAlert('Sign in successfully.', 'admin/index.php')</script>";
        } else {
    ...
    <div class="offcanvas-body">
        <div class="">
            <form action="" method="post">
                ...
                <button type="submit" class="btn" name="userSignUpBtn">Sign Up</button>
            </form>
        </div>
    </div>
</div>
...
<div class="offcanvas offcanvas-start" data-bs-backdrop="static" tabindex="-1" id="signIn" aria-labelledby="signInLabel">
    ...
    <div class="offcanvas-body">
        <div class="">
            ...
            <a href="#signUp" class="d-block my-2" data-bs-toggle="offcanvas" aria-controls="staticBackdrop">Don't have account yet? Click here!</a>
        </div>
    </div>
</div>
